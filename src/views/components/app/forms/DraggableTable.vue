<template>
  ...
  <div class="flex flex-row-reverse">
    <TableFilterButton v-if="columns.length">
      <ul>
        <draggable v-if="columns.length" class="w-full dragArea list-group" :list="columns" @change="log">
          <li v-for="(element, index) in columns" :key="element.name" @click="toggleVisible(index)"
            ...
            <div class="flex flex-row justify-between w-full space-x-2 select-none">
              <div>
                <i class="text-sm mr-2 text-[#337AB7]"
                  :class="[element.visible ? 'fas fa-circle-check' : 'far fa-circle']"></i>
                {{ element.label }}
              </div>
              <div><i class="text-sm text-gray-300 fas fa-grip-vertical cursor-grab"></i></div>
            </div>
          </li>
        </draggable>
      </ul>
  ...
  <!-- Таблица -->
  <table class="min-w-full mb-6 bg-white rounded shadow-md w-100">
    <thead class="bg-gray-100 rounded-t-sm">
      <draggable v-if="columns.length" tag="tr" class="w-full dragArea list-group" :list="columns" @change="log">
        <th v-for="(element, index) in columns" :key="element.name"
          :class="{ hidden: !element.visible, relative: true }"
          class="px-4 py-2 font-medium text-left border border-gray-300 cursor-pointer select-none"
          ...
          <template v-else>
            ...
            <span v-if="element.visible" class="absolute top-0 right-0 w-1 h-full resize-handle cursor-col-resize"
    ...
    <tbody>
      <tr v-if="sortedData.length === 0" class="text-center">
        <td class="px-4 py-2 border-gray-300 border-x" :colspan="columns.length">
          Нет данных
        </td>
      </tr>
      <tr v-for="(item, idx) in sortedData" :key="idx" class="transition-all cursor-pointer hover:bg-gray-100"
        :class="{ 'border-b border-gray-300': idx !== sortedData.length - 1 }" @click="(e) => itemClick(item, e)">
        <td v-for="(column, cIndex) in columns" :key="`${cIndex}_${idx}`" class="px-4 py-2 border-gray-300 border-x"
